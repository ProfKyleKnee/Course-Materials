...
// ---- mode-switch transition helpers (point/tangent/curves tween to their new positions instead
// of snapping) ----
...
// Curve arrays only interpolate point-by-point when both the outgoing and incoming curve have the
// same point count (i.e. both mixed-mode curves, or both the always-present live curve) -- when a
// curve is appearing/disappearing (ref/path curves only exist in mixed modes), there's nothing
// meaningful to morph from/to, so it snaps straight to the target instead of animating.
function lerpCurveArray(fromArr, toArr, t) {
  if (fromArr && toArr && fromArr.length === toArr.length && fromArr.length > 0) {
    return toArr.map((p, i) => lerpVec3(fromArr[i], p, t));
  }
  return toArr;
}
// The ref/path curves only exist in mixed modes, so a transition into/out of a mixed mode can't
// morph point-by-point (there's no "from" or "to" shape on the other side). Instead they fade:
// appearing curves hold the target shape and fade opacity in from 0, disappearing curves hold the
// outgoing shape and fade out to 0 -- only when both sides are mixed (fromLen === toLen) do they
// still morph positionally, at full opacity throughout, same as before.
function transitionCurveAndOpacity(fromArr, toArr, t) {
  const fromLen = fromArr ? fromArr.length : 0;
  const toLen = toArr ? toArr.length : 0;
  if (fromLen > 0 && toLen > 0 && fromLen === toLen) {
    return { points: lerpCurveArray(fromArr, toArr, t), opacity: 1 };
  }
  if (fromLen === 0 && toLen > 0) return { points: toArr, opacity: t };
  if (fromLen > 0 && toLen === 0) return { points: fromArr, opacity: 1 - t };
  return { points: toArr, opacity: 0 };
}
function cloneCurveState(s) {
  return {
    t1: s.t1.clone(),
    t2: s.t2.clone(),
    pt: s.pt.clone(),
    refCurve: s.refCurve.map((p) => p.clone()),
    liveCurve: s.liveCurve.map((p) => p.clone()),
    pathCurve: s.pathCurve.map((p) => p.clone()),
  };
}
function PartialDerivativeMockup() {
  ...
  // curveWorldRef holds the current TARGET state (where the point/tangent/curves should end up);
  // displayCurveRef holds what's actually drawn each frame -- during a mode-switch transition these
  // differ, with displayCurveRef tweening from its old value toward curveWorldRef (see
  // transitionRef below and its use in animate()).
  ...
  const transitionRef = useRef({ active: false, start: 0, duration: 450, from: null, to: null });
  const prevModeRef = useRef("fx");
  ...
  useEffect(() => {
    ...
    // Both fade in/out when transitioning into/out of a mixed mode (see transitionCurveAndOpacity
    // and its use in animate()) -- transparent must be on for their material's opacity to actually
    // blend instead of being ignored.
    m.refCurve.material.transparent = true;
    m.pathCurve.material.transparent = true;
    m.tangent = makeRibbonMesh(2, INDIGO);
    // Unlike every other ribbon, the tangent line always renders on top regardless of what's in
    // front of it in 3D space -- a straight tangent line only touches the true surface at the
    // point of tangency, so its extended ends legitimately dip below the (curved) surface nearby,
    // and strict depth-tested occlusion was hiding exactly the part students most need to see.
    // depthWrite is also off so this doesn't corrupt the depth buffer for anything drawn after it.
    m.tangent.material.depthTest = false;
    m.tangent.material.depthWrite = false;
    m.tangent.renderOrder = 999;
    ...
    const animate = () => {
      ...
      let refOpacity = 1,
        pathOpacity = 1;
      if (trans.active) {
        const raw = Math.min(1, (performance.now() - trans.start) / trans.duration);
        const te = easeInOutCubic(raw);
        const refT = transitionCurveAndOpacity(trans.from.refCurve, trans.to.refCurve, te);
        const pathT = transitionCurveAndOpacity(trans.from.pathCurve, trans.to.pathCurve, te);
        refOpacity = refT.opacity;
        pathOpacity = pathT.opacity;
        display = {
          t1: lerpVec3(trans.from.t1, trans.to.t1, te),
          t2: lerpVec3(trans.from.t2, trans.to.t2, te),
          pt: lerpVec3(trans.from.pt, trans.to.pt, te),
          refCurve: refT.points,
          liveCurve: lerpCurveArray(trans.from.liveCurve, trans.to.liveCurve, te),
          pathCurve: pathT.points,
        };
        displayCurveRef.current = display;
        if (raw >= 1) {
          trans.active = false;
          // A disappearing ref/path curve intentionally keeps rendering its old (non-empty) shape
          // throughout the fade so there's something to fade out -- but that means it never
          // actually converges to the target's empty array on its own. Snap straight to the real
          // target on completion so a finished transition can't leave a stale curve sitting at
          // full opacity (opacity itself resets to 1 by default once no transition is active).
          displayCurveRef.current = trans.to;
        }
      } else {
        display = displayCurveRef.current;
      }
      const { t1, t2, pt, refCurve, liveCurve, pathCurve } = display;
      const axes = axisWorldRef.current;
      const refDist = refDistRef.current;
      // Visibility follows whether there's actually anything to draw (not just the current mode's
      // `mixed` flag) so a curve stays visible and rendered mid-fade even after its owning mode has
      // already changed underneath it -- see transitionCurveAndOpacity above.
      m.refCurve.visible = refCurve.length > 0;
      m.pathCurve.visible = pathCurve.length > 0;
      m.refCurve.material.opacity = refOpacity;
      m.pathCurve.material.opacity = pathOpacity;
      if (refCurve.length)
  ...
  useEffect(() => {
    ...
    // Only tween on an actual mode switch (a discrete jump between pill buttons) -- dragging the
    // slider or typing a fixed value already moves continuously, so those should keep updating the
    // display state instantly rather than lagging behind a tween.
    if (prevModeRef.current !== mode) {
      transitionRef.current = {
        active: true,
        start: performance.now(),
        duration: 450,
        from: cloneCurveState(displayCurveRef.current),
        to: curveWorldRef.current,
      };
    } else {
      transitionRef.current.active = false;
      displayCurveRef.current = curveWorldRef.current;
    }
