...
`RelatedRate-Wiring` branch (not yet merged to `main`), plus a printable QR code
(`assets/qr-code-seal.png` / `qr-code-seal-black.png`) linking to the site's own GitHub Pages URL,
for use outside the site entirely — on the syllabus and the printed semester schedule posted
outside the professor's office — so neither file is referenced by any HTML/JS. Generated by a
one-off scratch Node script (`qrcode` + `sharp`, not committed — this repo has no build tooling of
its own, see §5) at error-correction level H specifically so a centered circular badge (the full
`logo-seal-white.svg` K² seal — curved `PROFESSOR`/`MATHEMATICS` ring and all, not just the bare
monogram — on a `#003366` navy fill matching the QR module color itself, ringed by a white buffer
so the modules don't crowd the badge) can safely cover a meaningful fraction of the code without
hurting scannability. Worth knowing if this ever needs regenerating: `sharp`'s bundled librsvg
silently drops `<textPath>` text when rasterizing `logo-seal-white.svg` directly (no error, the
...
| `qr-code-seal.png`, `qr-code-seal-black.png` | Printable QR code linking to the site's GitHub Pages home page, with the full `logo-seal-white.svg` K² seal centered on a navy badge — navy/accent-blue and pure-black module-color variants. Not referenced by any HTML/JS; for print use outside the site (syllabus, office-door schedule) |
